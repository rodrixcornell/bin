...
then 
	...
	git log --pretty=format:"%h\\ %s\\ [%cd] - [%cn]" --decorate --stat --since=6.weeks --author $1 >> ~/bkp/${logs}__$1.txt
...
echo
	...
	git log --pretty=format:"%h\\ %s\\ [%cd] - [%cn]" --decorate --stat --since=6.weeks >> ~/bkp/${logs}.txt
